...
public class SalesControllerJFX implements Initializable {
    ...
    @FXML
    private JFXTextField sBillNo, sPrescribedBy, sAddress, sProductId, sExpireDate, sMrp, sQuantityEntered, sAmount, sQuantityLeft, sNetTotal, sDiscount, cashReceived, sReturn;
    ...
    @FXML
    private JFXTextField afterRounding, rounding;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        cashReceived.textProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if (cashReceived.getText().trim().isEmpty() || cashReceived.getText() == null) {
                ...
                Double d = Math.abs(Double.valueOf(afterRounding.getText()) - Double.valueOf(cashReceived.getText()));
        ...
        sDiscount.textProperty().addListener((observable, oldValue, newValue) -> {
            if (sDiscount.getText().trim().isEmpty() || sDiscount.getText() == null) {
                sNetTotal.setText(sTotal.getText());
            } else {
                ...
                sNetTotal.setText(new DecimalFormat("####.##").format(d));
            }
        });

        sNetTotal.textProperty().addListener(((observable, oldValue, newValue) -> {
            String string_temp = new Double(sNetTotal.getText()).toString();
            String string_form = string_temp.substring(0,string_temp.indexOf('.'));
            double t = Double.valueOf(string_form);
            Double round = Double.valueOf(sNetTotal.getText()) - t;
            rounding.setText(new DecimalFormat("##.##").format(round));
            afterRounding.setText(String.valueOf(Math.round(Double.valueOf(sNetTotal.getText()))));
        }));
...
//            preparedStatement.executeUpdate();
    ...
    void onClickSaveButton(ActionEvent event) {
        ...
        cb.setTotalAmount(Double.valueOf(afterRounding.getText()));
